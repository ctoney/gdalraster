...
system.time(lapply(0:(nrows - 1), process_row))
#>    user  system elapsed
#> 119.548   4.623 124.804
```

The next test reads by block on the original tiled raster as distributed by LANDFIRE. To read by square blocks we need to calculate the row/column offsets and x/y sizes for each tile, including the incomplete tiles along the right and bottom edges.
...
system.time({
  for (i in seq_len(nrow(blocks))) {
    ds$readBlock(band = 1, blocks[i, 1], blocks[i, 2])
    }
  })
#>    user  system elapsed 
#> 121.020   8.720 133.492
```


*** TODO: text for read by chunk ***


```r
## Test 4
## original tiled raster, read chunks defined as 32 consecutive blocks
...
system.time({
  for (i in seq_len(nrow(chunks))) {
    ds$readChunk(band = 1, chunks[i, ])
    }
  })
#>    user  system elapsed 
#> 106.591   9.037 119.375
```
